...
from scripts.mbw_only.merge_block_weighted_only import merge_only
...
def on_ui_tabs():
    with gr.Column():
        ...
        with gr.Row():
            merge_targets_0 = gr.CheckboxGroup(label="Merge Only (Structure)", choices=[
                    "CLIP",
                    "U-Net",
                    "VAE",
                    "Other"
        ...
        with gr.Row():
            with gr.Column():
                gr.HTML("Merge only key include selected keyword")
                with gr.Row():
                    merge_targets_1 = gr.CheckboxGroup(label="Merge Only (Weight/bias)", choices=[
                            ".weight",
                            ".bias"
                            ], value=[])
                    merge_targets_2 = gr.CheckboxGroup(label="Merge Only", choices=[
                            ".time_embed",
                            ".in_layers",
                            ".emb_layers",
                            ".out_layers",
                    ...
                    merge_targets_3 = gr.CheckboxGroup(label="Merge Only (transformer)", choices=[
                            ".attn1.",
                            ".ff.",
                            ".attn2.",
                            ".norm1.",
    ...
    def onclick_btn_do_merge_block_weighted(
        ...
        radio_position_ids,
        merge_targets_0, merge_targets_1, merge_targets_2, merge_targets_3
    ):
        ...
        result, ret_message = merge_only(weights=_weights, model_0=model_A, model_1=model_B, allow_overwrite=chk_allow_overwrite,
            ...
            merge_target_st=merge_targets_0,
            merge_target_key=merge_targets_1 + merge_targets_2 + merge_targets_3
            )
