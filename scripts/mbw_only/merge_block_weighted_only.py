...
def merge_only(weights:list, model_0, model_1, device="cpu", base_alpha=0.5,
    ...
    print("  merging ...")
    dprint(f"    merge_target_st  [{len(merge_target_st)}]: {merge_target_st}", verbose)
    dprint(f"    merge_target_key [{len(merge_target_key)}]: {merge_target_key}", verbose)
    ...
    for key in (tqdm(theta_0.keys(), desc="Stage 1/2")):
        ...
        if key.startswith("cond_stage_model."):  # CLIP
            if "CLIP" not in merge_target_st:
                dprint(f"    skip: CLIP {key}", verbose)
                count_keys_ignored.append(f"{key}, CLIP_0")
                continue
        elif key.startswith("model.diffusion_model."):  # UNET
            if "U-Net" not in merge_target_st:
                dprint(f"    skip: U-Net {key}", verbose)
                count_keys_ignored.append(f"{key}, U-Net_0")
                continue
        elif key.startswith("first_stage_model."):  # VAE
            if "VAE" not in merge_target_st:
                dprint(f"    skip: VAE {key}", verbose)
                count_keys_ignored.append(f"{key}, VAE_0")
                continue
        else:
            if "Other" not in merge_target_st:
                dprint(f"    skip: Other {key}", verbose)
                count_keys_ignored.append(f"{key}, Other_0")
                continue
        ...
        if "model" in key and key in theta_1:
            ...
            if len(merge_target_key) > 0:
                if not any((x in key for x in merge_target_key)):
                    print(f"  skip: not target key: {key}")
                    count_keys_ignored.append(f"{key}, No-Key_0")
                    continue
                print(f"  target: {key}")
            ...
            if theta_0[key].size() != theta_1[key].size():
                print(f"{key}: theta_0:{theta_0[key].size()}, theta_1:{theta_1[key].size()}")
                count_keys_ignored.append(f"{key}, {theta_0[key].size()}, {theta_1[key].size()}")
                
            else:
                theta_0[key] = (1 - current_alpha) * theta_0[key] + current_alpha * theta_1[key]
                count_keys_merged.append(f"{key}, key_0")
                count_memo.append(f"{key}, key_0, {theta_1[key]}")
    ...
    for key in tqdm(theta_1.keys(), desc="Stage 2/2"):
        ...
        if key.startswith("cond_stage_model."):  # CLIP
            if "CLIP" not in merge_target_st:
                count_keys_ignored.append(f"{key}, CLIP_1")
                continue
        elif key.startswith("model.diffusion_model."):  # UNET
            if "U-NET" not in merge_target_st:
                count_keys_ignored.append(f"{key}, U-Net_1")
                continue
        elif key.startswith("first_stage_model."):  # VAE
            if "VAE" not in merge_target_st:
                count_keys_ignored.append(f"{key}, VAE_1")
                continue
        else:
            if "Other" not in merge_target_st:
                count_keys_ignored.append(f"{key}, Other_1")
                continue
        ...
        if "model" in key and key not in theta_0:
            ...
            if len(merge_target_key) > 0:
                if not any((x in key for x in merge_target_key)):
                    count_keys_ignored.append(f"{key}, No-Key_1")
                    continue
                print(f"  target: {key}")

            dprint(f"  key   : {key}", verbose)
            theta_0.update({key:theta_1[key]})
            count_keys_merged.append(f"{key}, key_1")
    ...
    print(f"  {len(count_keys_merged)} keys merged. {len(count_keys_ignored)} keys ignored.")
    print("Saving...")

    try:
        import json
        with open(output_file + "_keys.json", "w") as f:
            data = [
                    {"merged": count_keys_merged},
                    {"ignored": count_keys_ignored},
                    {"memo": count_memo}
                ]
